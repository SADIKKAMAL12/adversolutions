...
DROP POLICY IF EXISTS "Public read inventory products" ON inventory_products;
DROP POLICY IF EXISTS "Public read media buyers" ON media_buyers;
DROP POLICY IF EXISTS "Public read payment methods" ON payment_methods;
DROP POLICY IF EXISTS "Public read business types" ON business_types;
DROP POLICY IF EXISTS "Public read announcements" ON announcements;
DROP POLICY IF EXISTS "Allow all users" ON users;
DROP POLICY IF EXISTS "Allow all transactions" ON transactions;
DROP POLICY IF EXISTS "Allow all ad_account_requests" ON ad_account_requests;
DROP POLICY IF EXISTS "Allow all inventory_lines" ON inventory_lines;
...
DROP TABLE IF EXISTS media_buyers CASCADE;
...
DROP TABLE IF EXISTS ad_account_requests CASCADE;
...
CREATE TABLE transactions (
  ...
  type TEXT NOT NULL, method TEXT, amount NUMERIC(12,2) NOT NULL,
  status TEXT NOT NULL DEFAULT 'pending', date TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

CREATE TABLE ad_account_requests (
  id TEXT PRIMARY KEY, user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  platform TEXT NOT NULL, account_name TEXT, timezone TEXT, currency TEXT,
  websites JSONB, page_links JSONB, bm_id TEXT, business_name TEXT,
  business_type TEXT, business_email TEXT, status TEXT NOT NULL DEFAULT 'pending',
  amount NUMERIC(12,2), created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

CREATE TABLE inventory_products (
  id TEXT PRIMARY KEY, platform TEXT NOT NULL, type TEXT NOT NULL,
  title TEXT NOT NULL, description TEXT, price NUMERIC(12,2) NOT NULL,
  country TEXT, created TEXT
);

CREATE TABLE inventory_lines (
  id TEXT PRIMARY KEY, product_id TEXT REFERENCES inventory_products(id) ON DELETE CASCADE,
  email TEXT NOT NULL, password TEXT NOT NULL, twofa TEXT,
...
CREATE TABLE projects (
  id TEXT PRIMARY KEY DEFAULT gen_random_uuid()::text,
  client_name TEXT, client_email TEXT, buyer_name TEXT, budget NUMERIC(12,2),
  progress INTEGER DEFAULT 0, status TEXT DEFAULT 'active',
  updates JSONB DEFAULT '[]'::jsonb, files JSONB DEFAULT '[]'::jsonb,
  messages JSONB DEFAULT '[]'::jsonb, created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

CREATE TABLE orders (
  id TEXT PRIMARY KEY, user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  type TEXT NOT NULL, platform TEXT, item TEXT, amount NUMERIC(12,2) NOT NULL,
  status TEXT NOT NULL DEFAULT 'pending', date TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE deposits (
  id TEXT PRIMARY KEY, user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  method TEXT NOT NULL, amount NUMERIC(12,2) NOT NULL, account TEXT, proof TEXT,
  status TEXT NOT NULL DEFAULT 'pending', date TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

CREATE TABLE media_buyers (
  id SERIAL PRIMARY KEY, name TEXT NOT NULL, email TEXT, avatar TEXT,
  speciality TEXT, platforms JSONB, experience TEXT, spent TEXT,
  rate NUMERIC(12,2), rating NUMERIC(3,2), reviews INTEGER DEFAULT 0,
  orders INTEGER DEFAULT 0, status TEXT NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'approved', 'rejected')),
  reject_reason TEXT, joined TEXT, portfolio TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

CREATE TABLE payment_methods (
  id TEXT PRIMARY KEY, name TEXT NOT NULL, account TEXT, active BOOLEAN NOT NULL DEFAULT true
);

CREATE TABLE business_types (id SERIAL PRIMARY KEY, name TEXT NOT NULL UNIQUE);

CREATE TABLE support_tickets (
  id SERIAL PRIMARY KEY, user_id UUID REFERENCES users(id) ON DELETE CASCADE,
  user_email TEXT, subject TEXT NOT NULL, message TEXT NOT NULL,
  status TEXT NOT NULL DEFAULT 'open', created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

CREATE TABLE announcements (
  id SERIAL PRIMARY KEY, icon TEXT, title TEXT NOT NULL, body TEXT NOT NULL,
  date TEXT, created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- STEP 5: Seed users (password = "password" for all users, "admin2024" for admin)
...
-- STEP 9: Seed deposits, orders, payment methods, business types, announcements
...
INSERT INTO payment_methods (id, name, account, active) VALUES
('pm-1', 'Payoneer', 'adver@solution.com', true),
('pm-2', 'Wise', 'adver@solution.com', true),
('pm-3', 'USDC (TRC20)', 'TY8c...b3fD', true),
('pm-4', 'Binance', '456789852', true),
('pm-5', 'Bank Transfer', 'XXXX XXXX XXXX 1234', true),
('pm-6', 'Local Moroccan Banks', 'Contact support', true),
('pm-7', 'Other', 'Contact support', true);
...
CREATE POLICY "Public read media buyers" ON media_buyers FOR SELECT USING (true);
CREATE POLICY "Public read payment methods" ON payment_methods FOR SELECT USING (true);
CREATE POLICY "Public read business types" ON business_types FOR SELECT USING (true);
CREATE POLICY "Public read announcements" ON announcements FOR SELECT USING (true);
CREATE POLICY "Allow all users" ON users FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "Allow all transactions" ON transactions FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "Allow all ad_account_requests" ON ad_account_requests FOR ALL USING (true) WITH CHECK (true);
CREATE POLICY "Allow all inventory_lines" ON inventory_lines FOR ALL USING (true) WITH CHECK (true);
